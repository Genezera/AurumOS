...
pub async fn run(scores: EdgeScores, tx: watch::Sender<Vec<String>>, seed: Vec<String>, bus: EventBus) {
    let mut exploration_cursor: usize = 0;
    let mut candidate_pool: Vec<String> = seed.clone();
    let mut first_round = true;

    loop {
        match fetch_candidate_pool().await {
            Ok(list) if !list.is_empty() => {
                tracing::info!(total = list.len(), "universo de símbolos: lista de candidatos atualizada (Bybit linear, por volume 24h)");
                candidate_pool = list;
            }
            Ok(_) => {
                tracing::warn!("universo de símbolos: Bybit retornou lista vazia, mantendo candidatos anteriores");
            }
            Err(e) => {
                tracing::warn!(error = %e, "universo de símbolos: falha ao buscar instrumentos da Bybit, mantendo candidatos anteriores");
            }
        }

        let active = rotate(&scores, &candidate_pool, &seed, first_round, &mut exploration_cursor);
        first_round = false;
        persist(&active, &scores);
        tracing::info!(total = active.len(), "universo de símbolos: rotação concluída");
        bus.emit(DashboardEvent::symbol_universe(active.len(), top_ranked(&active, &scores, 12)));
        if tx.send(active).is_err() {
            tracing::warn!("universo de símbolos: nenhum consumidor ouvindo mais, encerrando tarefa de rotação");
            return;
        }
...
fn persist(active: &[String], scores: &EdgeScores) {
    let ranked = ranked_symbols(active, scores);
    let path = format!("{}/data/active_symbols.json", env!("CARGO_MANIFEST_DIR"));
...
async fn fetch_candidate_pool() -> anyhow::Result<Vec<String>> {
    let client = reqwest::Client::builder()
        ...
        .build()?;
    let resp = client.get(BYBIT_TICKERS_URL).send().await?.error_for_status()?;
